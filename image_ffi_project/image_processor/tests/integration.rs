...
fn integration_test_mirror_and_blur() {
    ...
    assert!(
        params_mirror.exists(),
        "Mirror params not found at image_ffi_project/test/params_mirror.json"
    );
    assert!(
        params_blur.exists(),
        "Blur params not found at image_ffi_project/test/params_blur.json"
    );
    ...
    let out_dir = env::temp_dir()
        .join("output")
        .join("image_ffi_integration_test");
