...
#[derive(Debug)]
struct LogIterator<R: std::io::Read + std::fmt::Debug + 'static> {
    lines: std::io::Lines<std::io::BufReader<R>>,
}

impl<R: std::io::Read + std::fmt::Debug + 'static> LogIterator<R> {
    fn new(r: R) -> Self {
        Self {
            lines: std::io::BufReader::with_capacity(4096, r).lines(),
        }
    }
}
