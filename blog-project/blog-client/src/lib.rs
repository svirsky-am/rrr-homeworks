...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Post { pub id: i64, pub title: String, pub content: String, pub author_id: i64, pub created_at: String, pub updated_at: String }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ListPostsResponse { pub posts: Vec<Post>, pub total: i64, pub limit: i64, pub offset: i64 }
...
impl BlogClient {
    ...
    pub async fn new_grpc(grpc_url: String) -> Result<Self, BlogClientError> {
        let channel = tonic::transport::Channel::from_shared(grpc_url)?.connect().await?;
    ...
    pub fn get_token(&self) -> Option<&String> { self.token.as_ref() }
    ...
    pub async fn register(&mut self, username: &str, email: &str, password: &str) -> Result<AuthResponse, BlogClientError> {
        let resp = match &mut self.transport {
            TransportClient::Http(c, url) => http_client::register(c, url, username, email, password).await?,
    ...
    pub async fn create_post(&self, title: &str, content: &str) -> Result<Post, BlogClientError> {
        let token = self.token.as_ref().ok_or_else(|| BlogClientError::Unauthorized("Not authenticated".into()))?;
        match &self.transport {
            TransportClient::Http(c, url) => http_client::create_post(c, url, token, title, content).await,
            TransportClient::Grpc(arc) => {
                let mut guard = arc.lock().await;
    ...
    // === UPDATE POST ===
    pub async fn update_post(&self, id: i64, title: Option<&str>, content: Option<&str>) -> Result<Post, BlogClientError> {
        let token = self.token.as_ref().ok_or_else(|| BlogClientError::Unauthorized("Not authenticated".into()))?;
        match &self.transport {
            TransportClient::Http(c, url) => http_client::update_post(c, url, token, id, title, content).await,
            TransportClient::Grpc(arc) => {
                let mut guard = arc.lock().await;
    ...
    // === LIST POSTS ===
    pub async fn list_posts(&self, limit: Option<i64>, offset: Option<i64>) -> Result<ListPostsResponse, BlogClientError> {
        match &self.transport {
