...
fn auth_req<T>(token: &str, msg: T) -> Request<T> {
    ...
    r.metadata_mut().insert(
        "authorization",
        MetadataValue::try_from(&format!("Bearer {}", token)).expect("Invalid token"),
    );
    r
}

pub async fn register(
    c: &mut BlogServiceClient<tonic::transport::Channel>,
    u: &str,
    e: &str,
    p: &str,
    ...
    Ok(AuthResponse {
        ...
        user: r
            ...
            .ok_or(BlogClientError::Internal("No user".to_string()))?,
    })
}
...
pub async fn create_post(
    ...
    r.post
        ...
        .ok_or(BlogClientError::Internal("No post".to_string()))
}
...
pub async fn delete_post(
    ...
    let r = c
        .delete_post(auth_req(t, PostId { id }))
        .await?
        .into_inner();
    if r.success {
        Ok(())
    } else {
        Err(BlogClientError::Internal("Delete failed".to_string()))
    }
}
