...
export TOKEN_CLIENT=$(curl -s -X POST http://localhost:8080/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username": "checker", "email": "user@example.com", "password": "secure123"}' \
  | jq -r '.token')
...
#  3. Создание поста  
curl -v -X POST http://localhost:8080/api/posts/p_test \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN_CLIENT" \
  -d '{"title": "test_data", "content": "test_content"}'
