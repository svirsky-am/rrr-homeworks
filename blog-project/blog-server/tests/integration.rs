...
use blog_server::{run_server, ServerConfig, init_services};
use reqwest::Client;
...
impl AuthenticatedClient {
    ...
    async fn update_post(&self, id: i64, title: Option<&str>, content: Option<&str>) -> reqwest::Response {
        let mut req = self.client
...
async fn test_full_flow_single_client() {
    ...
    assert!(list.posts.iter().any(|p| p.id == created_post.id), "Created post should be in list");
    ...
    let update_resp = auth_client.update_post(created_post.id, Some("Updated Title"), None).await;
    ...
    let list_after_resp: ListPostsResponse = list_after.json().await.expect("Should parse empty list");
    assert_eq!(list_after_resp.total, 0, "Posts list should be empty after deletion");
...
async fn test_concurrent_clients() {
    ...
    let tasks = (0..3).map(|i| {
        ...
        tokio::spawn(async move {
            ...
            // === Создание поста ===
            let create = auth_client.create_post(
                &format!("Post from client {}", i),
                &format!("Content from client {}", i)
            ).await;
            
            assert_eq!(create.status(), 201, "Client {} create post should succeed", i);
    ...
    assert!(list_resp.total >= 3, "Should have at least 3 posts, got {}", list_resp.total);
...
async fn test_unauthorized_access() {
    ...
    assert_eq!(create.status(), 401, "Create post without token should return 401");
    ...
    assert_eq!(post_resp.status(), 201, "User1 should be able to create post");
    ...
    assert_eq!(update.status(), 403, "User2 should not be able to update User1's post");
    
    // === 5. User2 пытается удалить пост User1 -> 403 Forbidden ===
    let delete = client2.delete_post(post.id).await;
    assert_eq!(delete.status(), 403, "User2 should not be able to delete User1's post");
    
    // === 6. User1 может обновить свой пост ===
    let update_own = client1.update_post(post.id, Some("Updated by Owner"), None).await;
    assert_eq!(update_own.status(), 200, "User1 should be able to update own post");
    
    // === 7. User1 может удалить свой пост ===
    let delete_own = client1.delete_post(post.id).await;
    assert_eq!(delete_own.status(), 204, "User1 should be able to delete own post");
...
async fn test_posts_pagination() {
    ...
    for i in 0..5 {
        let resp = auth_client.create_post(&format!("Post {}", i), &format!("Content {}", i)).await;
        assert_eq!(resp.status(), 201);
    }
    
    // === Тест пагинации: limit=2, offset=0 ===
    let list1 = auth_client.client
