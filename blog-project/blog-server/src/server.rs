...
use sqlx::PgPool;
use std::sync::Arc;
use crate::{application, data, infrastructure, presentation, config::ServerConfig, blog};
...
pub async fn init_services(
    ...
    (auth_data, blog_data, jwt_data, grpc_auth_service, grpc_blog_service)
}
...
pub async fn run_server(
    config: ServerConfig,
) -> Result<StartedServer, std::io::Error>  {
    ...
    let http_server = HttpServer::new(move || {
        ...
        App::new()
            ...
            .service(presentation::public_scope()
                .service(
                    presentation::protected_scope()
    ...
    let handle = tokio::spawn(async move {
        tracing::info!("🌐 HTTP server listening on http://{}", actual_http_addr_for_log);
        tracing::info!("🔗 gRPC server listening on http://{}", actual_grpc_addr_for_log);
