...
В файл `src/lib.rs` добавлена оригинальная функция `normalize` из `reference_app` под названием `normalize_by_reference_app`, ускореенная функция `normalize_new_optimized`, а текущую реализацию оставим под названием `normalize_with_hot_fix`. Для функций добвлены bench'и для сравнения скорости работы.
